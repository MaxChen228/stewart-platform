...
- **安全殼守界（soft-knee 投影，2026-07-02 取代舊 IK-valid 凍結）**：目標 pose 沿「home→目標」6D 射線做無記憶投影回綠色安全殼（Type-I 徑向裕度 1.25× ∧ σmin(Jx)≥0.15 ∧ κ(Ja)≤12 ∧ 物理四閘）——殼內 knee(0.8) 比例恆等 1:1（零時域延遲）、外側 tanh 壓縮漸近貼邊滑牆不凍結；dot 標紅=夾持生效。實作 `workspace_envelope.makeSafeProjector`，**server `manualPfUpdateTarget` 為三來源主閘**（env `PF_SAFE_KNEE`/`PF_SAFE_CLAMP=0` 停用；knee=1=硬牆），phone.html/live.html(desktop/sim) 本地同一投影使顯示=下發。session executor 刻意不經此閘。韌體 `followStep` 速限 + `VF` 仍是最終硬安全閘。
