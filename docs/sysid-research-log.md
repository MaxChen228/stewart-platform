...
## 2026-06-27 — 操作預設改為 auto-return，HOME/RELEASE 避免混用 FOLLOW

使用者確認後續操作記憶：**回授預設使用 SERVO42D auto-return (`A1`)**，輪詢 (`A0`) 只作明確診斷/對照條件。
即時狀態以 `/api/latest` 的 `ar/cus/bus.per` 判斷：`ar>0`、`cus=0`、六顆 `bus.per` 有計數代表 auto-return 生效。
在 MCP2515 上仍需監看 `ef/rxDrop/tx`，必要時調慢 `AR`；但不要再把「預設輪詢」當一般運行假設。

同日操作觀察：使用 Web UI 打開跟隨模式後按 HOME 再 RELEASE，波形較不順；關掉跟隨模式後同樣 HOME/RELEASE
明顯更平滑。程式路徑對照：

- FOLLOW on：`poseHome()` / `poseRelease()` 先同步滑桿與 ghost，`poseUpdateGhost()` 會經 `streamPF()` 送 `PF`，
  由韌體 `followStep()` 的低通/速限濾波器接管。
- FOLLOW off：按鈕送單次 `P ... ms`，HOLD 路徑走韌體 `holdStart -> holdTarget` 的 smoothstep。

暫定判斷：離散 HOME/RELEASE 更適合 FOLLOW off 的 `P` smoothstep 路徑；FOLLOW/PF 保留給連續拖曳或 motion library。
後續若要改善 UI，可考慮在 HOME/RELEASE 按鈕下，即使 FOLLOW on 也先 `FOLLOW 0` 後送 `P`，或新增明確的「跟隨式 HOME」
與「軌跡式 HOME」兩種命令，避免隱性路徑切換造成誤判。

## 2026-06-27 — 舊 Stability score 退役，改用 lifecycle/raw waveform evaluation

### 觸發

Program 00 的 Motor 6-axis waveform 視覺上明顯失敗：takeoff/landing 有 staircase、lag、single-sample 大跳動，
但舊 `stability_score.js` 仍給出約 96 分。原始檢查發現舊 scoring window 只看 `recording -> landing`
中間約 1.7s 的平坦區，且三次 queue 的該窗口 `commandCount=0`，等於把全流程最差的 takeoff/landing 排除在總分外。

### 決策

正式退役舊 `sysid/stability_score.js` 與 `.stability.json` artifact。新正式評測系統為
`sysid/workspace_evaluation.js`，artifact 為 `.evaluation.json`，server、queue、leaderboard、runs UI、
workspace reports、Z sweep summary 全部改讀新 evaluation。

### 新評測原則

- full lifecycle 優先：takeoff、recording/block、close loop、landing、release 都在同一 run 的原始資料中被看見。
- 視覺 gate 優先：max motor step、P99 motor step、cross-axis high-pass peak、worst motor/axis 會直接進 verdict。
- block/window 與 lifecycle 分開：中間 hold/block 可以乾淨，但不能讓 takeoff/landing 的失敗被總分遮掉。
- 健康指標不洗白動作：CAN EFLG、rxDrop、ok/fk failure 只作資料品質與風險，不能把糟糕波形評成好。
- 後續 controller/MPC/RL 比較必須先通過 evaluation 的 raw waveform sanity gate，再討論模型或控制律。

## 2026-06-28 — Evaluation 不再輸出 scalar score，改為 execution feature report

### 觸發

使用者指出：即使修掉舊 scoring window，任何單一分數都容易讓人迷失，開始壓縮一次執行的結構，
而不是閱讀原始資料特徵。一次 run 應該被看成一組 phase/event/health/waveform 特徵，而不是一個標量。

### 修正

- `.evaluation.json` 移除 `quality.score`，server `/api/runs` 也不再回傳 `run.score`。
- Run UI、Workspace run mini panel、queue report、program feature report、Workspace deep/waveform report 全部移除 score 欄。
- `program:leaderboard` 改名為 `program:features`；輸出最新同 program runs 的特徵，不再排序最佳分數。
- 報告核心欄位改成：`verdict`、max motor step、P99 motor step、cross-axis peak、worst motor、takeoff/landing phase peaks、CAN EFLG/rxDrop。

### 新規則

後續分析 program00 時，不用分數作優化目標。每次執行必須直接閱讀 feature report、波形圖與 raw event 附近樣本，
先問「哪個 phase、哪顆 motor、哪個 pose axis、哪個 CAN/loop 狀態」再談控制律。

## 2026-06-28 — Workspace lifecycle 改為 staged takeoff/landing

### 觸發

三次 `program00_queue_ar20_purep` 的 feature report 都顯示最差事件集中在 takeoff/landing，且 worst motor 多為 M6。
典型 landing event 會在約 29ms 內跳 9-28deg，同時 pose 從 high-Z HOME 直接往 low-Z + yaw 65deg 的 landing pose
走。這表示 hold block 本體不是主要問題；真正失敗的是 lifecycle path 把大 Z 變化與大 yaw 變化綁成同一條直線。

### 修正

Workspace runner 預設 `trialDefaults.lifecyclePath = staged`：

- takeoff：先在 landing yaw 下升到 HOME 高度，再在高處回正 yaw 到 HOME。
- landing：先在 HOME 高度轉到 landing yaw，再下降到 landing Z。
- phase 會記成 `takeoff z`、`takeoff orient`、`landing orient`、`landing z`，讓 evaluation 的 top raw events 能直接指出是哪個 stage 失敗。

可用 `--lifecycle-path direct` 明確跑舊路徑作 A/B 對照，但一般 program00 平滑化實驗不應再默默使用 direct lifecycle。

## 2026-06-28 — 更正：撤回 staged lifecycle，改走短路徑 minimum-jerk

### 事故

`program00_staged_purep_ar20_*` 三次實測後平台倒掉。資料顯示 staged 方案把原本 `landMs=4000ms`
拆成 `landing orient 2200ms` + `landing z 1800ms`，等於把 93mm Z 下降壓縮到 1.8s。最差 raw events 集中在
`landing z`，M3/M4/M1 出現 14-17deg single-step，FK pose 也進入大 roll/pitch 偏斜。

### 更正

撤回 staged lifecycle。把 Z 與 yaw 拆成兩段會拉長幾何路徑，且容易把能量集中到最後下降段；這不是「平滑」。
Workspace lifecycle 回到單一短路徑 `P home` / `P landing`。

新的控制方向不是再改 waypoint，而是升級軌跡生成器：
