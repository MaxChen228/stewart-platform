...
function rms(pts) { if (!pts.length) return 0; const m = pts.reduce((a, b) => a + b[1], 0) / pts.length;
  return Math.sqrt(pts.reduce((a, b) => a + (b[1] - m) ** 2, 0) / pts.length); }

// σ：局部極值絕對幅 → ln 回歸斜率。r² 一併回報（低=非乾淨指數）
function sigma(pts) {
  if (pts.length < 12) return { s: NaN, r2: 0, n: 0 };
  const m = pts.reduce((a, b) => a + b[1], 0) / pts.length;
  const amp = [];
