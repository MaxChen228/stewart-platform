...
function buildPlan(opts, home) {
  ...
  const plan = [
    ...
    { cmd: `P ${poseLine(home)} ${opts.homeMs}`, wait: opts.homeMs + 500, note: 'repeatable start from configured home' },
    { cmd: `P ${poseLine(target)} ${opts.ms}`, wait: opts.ms + opts.settleMs, note: `main heave step to relative +${opts.heave}` },
  ];
  if (opts.safeLand) {
    plan.push(
