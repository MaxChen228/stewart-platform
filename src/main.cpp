...
// ===== 輪詢讀取率（2026-07-02 與迴圈率解耦，POLL 指令）=====
// no-ack+輪詢定案後，0x35 問答=匯流排大頭（查詢63+回覆111 bits ×1.1 stuff ≈ 383µs/顆 @500k）。
// 舊行為「每 cycle 全讀 6 顆」把 bus 佔用綁死迴圈率（355Hz → 81%、讀等待 ~2.7ms 反過來鎖死迴圈率），
// 且靜止時單 cycle Δangle < 1 LSB(0.022°) → per-cycle vel 全是量化噪聲。
// 解耦後：讀取按 pollPeriodMs 排程、due 時仍全讀 6 顆（六角時間相干，FK 需要）；
// vel/積分類估計改走 encoder 時基（encUpdated/encDt）。mode 0/1 備援為逐 cycle 真閉環 → 強制每 cycle 讀。
constexpr uint16_t POLL_PERIOD_MIN_MS = 2;    // 500Hz；≤迴圈週期時退化回每 cycle 讀（=舊行為）
constexpr uint16_t POLL_PERIOD_MAX_MS = 200;  // 5Hz；再低觀測明顯鈍於 telemetry ~33Hz
uint16_t pollPeriodMs = 10;                   // boot 預設 100Hz/顆（bus ~23% @500k，覆蓋所有消費者頻寬需求）
...
// vel 估計走 encoder 時基（encUpdated/encDt）：輪詢解耦後 enc.angles 只在讀取 cycle 變化，
// 若仍用迴圈 dt，非讀取 cycle vel=0、讀取 cycle vel 超估 N 倍（N=迴圈率/讀取率）。
// 狀態只在有新資料時前進；修正輸出每 cycle 由現值計算（連續作用）。
static void applyHoldOutputDamping(float targets[NUM_MOTORS], bool encUpdated, float encDt) {
    if (holdDampGainSec <= 0.0f || holdDampMaxDeg <= 0.0f) {
    ...
    if (encUpdated && encDt > 0.0f) {
        float alpha = constrain(encDt / (HOLD_DAMP_TAU + encDt), 0.0f, 1.0f);
        for (int i = 0; i < NUM_MOTORS; i++) {
            float rawVel = (enc.angles[i] - holdDampPrevAngle[i]) / encDt;
...
static void applyHoldIntegral(float targets[NUM_MOTORS], bool active, bool encUpdated, float encDt) {
    if (!holdIntInit) resetHoldIntegral();

    // 積分走 encoder 時基：err 在兩次讀取之間本來就不變，只在有新資料時前進積分器；
    // 修正值每 cycle 疊加到命令（連續作用），積分量在數學上與逐 cycle 積分等價。
    if (encUpdated && encDt > 0.0f) {
        for (int i = 0; i < NUM_MOTORS; i++) {
            float vel = (enc.angles[i] - holdIntPrevAngle[i]) / encDt;
            holdIntPrevAngle[i] = enc.angles[i];               // 每次讀取更新，gate 關也更新避免 vel spike
            float err = holdAngles[i] - enc.angles[i];          // 命令 − 實際；droop→err>0
            bool gated = active
                      && fabsf(vel) < holdISettleDps            // 物理靜止（次要閘）
                      && fabsf(err) > holdIDeadbandDeg;          // 死區外才積分
            holdIntCorr[i] = holdInt[i].update(err, encDt, holdKi[i], holdIClampDeg, gated);
        }
    }
    for (int i = 0; i < NUM_MOTORS; i++) targets[i] += holdIntCorr[i];
}
...
struct RuntimeTuningNVS {
    ...
    float odMaxDeg;
    float odDeadbandDps;
    uint16_t pollMs;   // v2 追加（v1 blob 到 odDeadbandDps 為止，載入時遷移）
};

constexpr uint32_t RUNTIME_TUNE_MAGIC = 0x5254554E; // "RTUN"
constexpr uint16_t RUNTIME_TUNE_VERSION = 2;
// v1→v2 遷移依賴：v1 blob 長度 == pollMs 之前的所有欄位（含 padding）
static_assert(offsetof(RuntimeTuningNVS, pollMs) == 32, "RuntimeTuningNVS v1 layout changed");

static bool validRuntimeTuning(const RuntimeTuningNVS& cfg, bool hasPoll) {
    if (cfg.magic != RUNTIME_TUNE_MAGIC) return false;
    if (cfg.version != (hasPoll ? RUNTIME_TUNE_VERSION : (uint16_t)1)) return false;
    if (!(cfg.currentMa >= 100 && cfg.currentMa <= 3000 &&
          ...
          isfinite(cfg.odDeadbandDps) && cfg.odDeadbandDps >= 0.0f && cfg.odDeadbandDps <= 30.0f)) return false;
    if (hasPoll && (cfg.pollMs < POLL_PERIOD_MIN_MS || cfg.pollMs > POLL_PERIOD_MAX_MS)) return false;
    return true;
}
...
static uint16_t clampPollPeriodMs(float requestedMs) {
    float clampedMs = constrain(requestedMs, (float)POLL_PERIOD_MIN_MS, (float)POLL_PERIOD_MAX_MS);
    return (uint16_t)constrain((int)floorf(clampedMs + 0.5f), (int)POLL_PERIOD_MIN_MS, (int)POLL_PERIOD_MAX_MS);
}
...
static bool loadRuntimeTuningFromNVS() {
    ...
    prefs.end();
    // v1 blob 不含 pollMs（長度到 odDeadbandDps 為止）→ 其餘欄位照收、pollMs 沿用 boot 預設
    bool hasPoll = (len == sizeof(cfg));
    if (!(hasPoll || len == offsetof(RuntimeTuningNVS, pollMs)) || !validRuntimeTuning(cfg, hasPoll)) return false;
    ...
    if (hasPoll) pollPeriodMs = clampPollPeriodMs((float)cfg.pollMs);
    return true;
}
...
static void emitTelemetry(const TelemetrySnapshot& s) {
    ...
    teleAppend(line, sizeof(line), pos, "{\"a\":[%.2f,%.2f,%.2f,%.2f,%.2f,%.2f],"
                  ...
                  "\"ok\":%d,\"z\":%d,\"pos\":%d,\"cm\":%d,"
                  "\"ef\":%u,\"tx\":%u,\"rx\":%u,"
                  "\"t\":%u,\"dt\":%u,\"cus\":%u,\"lhz\":%.0f,\"ar\":%d,\"poll\":%u,"
                  "\"arAge\":[%u,%u,%u,%u,%u,%u],"
        ...
        s.eflg, s.tec, s.rec,
        s.t, s.dtMs, s.canReadUs, s.lhz, s.arMs, s.pollMs,
...
void dispatch(const String& cmd, bool fromNet = false) {
    ...
    if (cmd == "Z") {
        ...
        // 輪詢讀取率（Hz/顆，全讀 6 顆時間相干）：與迴圈率解耦，即設即存 NVS。
        // ≥迴圈率時退化回每 cycle 讀（=舊行為）；mode 0/1 備援不受此限（強制每 cycle）。
        float hz = 0.0f;
        if (sscanf(cmd.c_str(), "POLL %f", &hz) == 1 && hz > 0.0f) {
            uint16_t prevMs = pollPeriodMs;
            pollPeriodMs = clampPollPeriodMs(1000.0f / hz);
            bool saved = saveRuntimeTuningToNVS();
            int clamped = (hz > 1000.0f / POLL_PERIOD_MIN_MS || hz < 1000.0f / POLL_PERIOD_MAX_MS) ? 1 : 0;
            Out.printf("{\"status\":\"poll rate\",\"period_ms\":%d,\"hz\":%.3f,\"requested_hz\":%.3f,"
                       "\"min_period_ms\":%d,\"max_period_ms\":%d,\"clamped\":%d,\"prev_period_ms\":%d,\"saved\":%d}\n",
                       pollPeriodMs, 1000.0f / pollPeriodMs, hz,
                       POLL_PERIOD_MIN_MS, POLL_PERIOD_MAX_MS, clamped, prevMs, saved ? 1 : 0);
        } else {
            Out.println("{\"error\":\"usage POLL hz\"}");
        }
...
static void controlTick() {
    ...
    bool encUpdated = false; // 本 cycle enc.angles 是否有新資料（vel/積分類估計以此節流）
    float encDtS = dt;       // 新資料距上次讀取的真實間隔（encoder 時基，秒）
    if (autoReturnMode) {
        encUpdated = true;   // auto-return 連續刷新，encoder 時基 = 迴圈時基
        ...
        // 輪詢排程（2026-07-02 解耦）：讀取率由 POLL 決定，不再綁迴圈率。due 時全讀 6 顆
        // （六角時間相干）；非讀取 cycle 沿用上次值（「讀失敗保持上次角度」語意的排程化推廣）。
        // mode 0/1 備援是逐 cycle 真閉環（error/vel 直接吃 enc.angles）→ 強制退回每 cycle 讀。
        static uint32_t lastPollUs = 0;
        static int64_t  pollRaw[NUM_MOTORS] = {INT64_MIN, INT64_MIN, INT64_MIN, INT64_MIN, INT64_MIN, INT64_MIN};
        static int      pollOk = 0;
        static bool     pollStale = true;
        static uint32_t pollCanReadUs = 0;
        bool forceEveryCycle = posEnabled && !holdMode;
        bool due = forceEveryCycle || lastPollUs == 0 ||
                   (uint32_t)(nowUs - lastPollUs) >= (uint32_t)pollPeriodMs * 1000u;
        if (due) {
            // vel 用真實讀取間隔；上限 clamp 防長時間停輪詢（模式切換等）後巨大 Δt 壓扁速度估計
            float rawEncDt = lastPollUs ? (float)(uint32_t)(nowUs - lastPollUs) / 1e6f
                                        : (float)pollPeriodMs / 1000.0f;
            encDtS = fmaxf(nominalDt * 0.5f,
                           fminf(rawEncDt, (float)pollPeriodMs * 3.0f / 1000.0f + nominalDt));
            lastPollUs = nowUs;
            servos.flushReceiveBuffer();
            uint32_t _canT0 = micros();
            pollOk = servos.readAllRawEncoders(pollRaw);
            pollCanReadUs = micros() - _canT0;   // 讀 6 顆編碼器的純 CAN 來回時間（頻寬指紋核心）
            busTxQ += NUM_MOTORS;                // 匯流排佔用：每次輪詢送 6 筆 0x35 查詢（各保證一筆回覆）
            pollStale = (pollOk < NUM_MOTORS);   // 任一顆讀失敗 → enc.angles 保持舊值 → 凍結積分
            encUpdated = true;
        }
        for (int i = 0; i < NUM_MOTORS; i++) raw[i] = pollRaw[i];
        ok = pollOk;
        canReadUs = pollCanReadUs;               // 非讀取 cycle 沿報上次讀取耗時（頻寬指紋連續）
        encStale = pollStale;
    }
    ...
    if (posEnabled) {
        ...
        if (holdMode) {
            ...
            applyHoldIntegral(motorTargets, intActive, encUpdated, encDtS);   // 積分外環先疊命令（消 droop）
            applyHoldOutputDamping(motorTargets, encUpdated, encDtS);          // 阻尼後處理（看 enc 速度）
            float maxAbs = 0;
            for (int i = 0; i < NUM_MOTORS; i++) {
